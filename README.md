...
Claude Code の状態を手元で確認する個人用ツールとして始まり、現在は Codex 対応、依頼全体の推定進捗、忍者アニメーション、サブエージェント分身まで拡張しています。

アカウント機能、DB、ホスト型バックエンドは持たない小さな Windows 常駐ツールです。

## 設計判断の記録

開発中に積み上げた採用・却下・実測の記録は [docs/DESIGN_DECISIONS.md](docs/DESIGN_DECISIONS.md) にあります。現在仕様は README と source を正とし、この文書は「なぜそうしたか」の履歴として残しています。
